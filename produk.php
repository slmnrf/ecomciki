...
if (isset($_GET['kategori'])) {
    ...
    $sql_produk_home = $con->query("SELECT a.*, b.* FROM produk AS a, kategori AS b WHERE a.stok > 0 AND b.kd_kategori = a.kd_kategori AND b.nama_kategori LIKE '%$kategori%' ");
    ...
    $sql_produk_home = $con->query("SELECT a.*, b.* FROM produk AS a, kategori AS b WHERE a.stok > 0 AND b.kd_kategori = a.kd_kategori HAVING b.nama_kategori LIKE '%$barang%' OR a.nama_produk LIKE  '%$barang%'");
    $judul='<h4>Hasil pencarian: <i class="text-kategori">"'.$barang.'"</i></h4><hr>';
    }
} else {
    $sql_produk_home = $con->query("SELECT * FROM produk WHERE stok > 0 ORDER BY tgl_produk DESC");
}
